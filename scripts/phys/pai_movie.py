...
def _render_chunk(chunk_idxs):
    ...
    for lab in (ax.set_xlabel, ax.set_ylabel, ax.set_zlabel):
        lab("")
    ax.set_xlim(*st["ranges"][0])
    ax.set_ylim(*st["ranges"][1])
    ...
    for prefix in st["axis_keys"]:
        coll = Line3DCollection(
            [np.zeros((2, 3))], colors=st["colors"][prefix], linewidths=3, zorder=5
        )
    ...
    for k, i in chunk_idxs:
        ...
        for prefix in st["domains"]:
            ...
            scatters[prefix]._offsets3d = (coords[:, 0], coords[:, 1], coords[:, 2])
        for prefix in st["axis_keys"]:
            d = st["domains"][prefix]
        ...
        time_txt.set_text(f"t = {time_all[i] / 1000:.1f} ns  ({i} / {n_total})")
        buf = io.BytesIO()
...
def main():
    ...
    ap.add_argument(
        "--rotate", type=float, default=0.0, help="vrtenje kamere [°/frame], 0 = fiksna"
    )
    ap.add_argument(
        "--no-unwrap", action="store_true", help="ne popravljaj predznaka osi"
    )
    args = ap.parse_args()
    ...
    if u.trajectory.n_frames != len(time_all):
        raise SystemExit(
            f"trajektorija {u.trajectory.n_frames} frame-ov, "
    ...
    frame_tasks = [
        (k, i) for k, i in enumerate(np.arange(0, u.trajectory.n_frames, args.step))
    ]
    chunks = [
        frame_tasks[s : s + args.chunk] for s in range(0, len(frame_tasks), args.chunk)
    ]
    ...
    print(
        f"{n_frames} frame-ov, {len(chunks)} kupčkov, {n_workers} procesov", flush=True
    )
    done = 0
    if n_workers > 1:
